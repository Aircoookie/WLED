...
bool UsermodBME68X::readFromConfig(JsonObject& root) {
	...
	if (top.isNull()) {
		DEBUG_PRINT(WARN);
		DEBUG_PRINTLN(F(UMOD_DEBUG_NAME "No config found (using defaults)"));
		return false;
	}
	...
	configComplete &= getJsonValue(top[FPSTR(_namePublishChange)], 				settings.PublischChange, 					true	);		// Publish changed values only
	...
	configComplete &= getJsonValue(top["Sensors"][FPSTR(_nameIaq)], 			settings.decimals.iaq, 						-1		);		// deavtivated
