...
#define HumidityDecimals 1    // Number of decimal places in published humidity values
...
public:
  ...
  void loop()
  {
    ...
    {
      // Timers to fetch new temperature, humidity and pressure data at intervals
      timer = millis();
      ...
      if (timer - lastTemperatureMeasure >= TemperatureInterval * 1000 || mqttTemperaturePub == 0)
      {
        ...
        // Create string populated with user defined device topic from the UI, and the read temperature, humidity and pressure, then publish to MQTT server only if values have changed since the last publish.
