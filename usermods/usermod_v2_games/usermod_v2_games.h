...
typedef struct PongBall {
  ...
  void hit() {
    ...
    // hit right wall?
    if (x + size-1 >= SEGMENT.virtualWidth()-1) {
    ...
    // hit bottom wall? 
    if (y + size-1 >= SEGMENT.virtualHeight()-1) {
...
uint16_t mode_pongGame(void) { 
  ...
  if (SEGENV.call == 0) {
    ...
    racket_left->x = 0;
    racket_left->y = vH/2 - racket_left->size/2;
    racket_left->size = vH/4;
    racket_left->speed = 1;
    racket_left->dir_y = 0.18;
    racket_left->size = 3;

    racket_right->x = vW - 1;
    racket_right->y = vH/2 - racket_left->size/2;
    racket_right->size = vH/4;
    ...
    uint32_t color = BLUE;//ColorFromPalette(SEGPALETTE, 158, 255, LINEARBLEND);
    ...
    // Serial.print(ball->y);
    // Serial.println();
    // SEGMENT.setPixelColorXY(previousX, previousY, BLACK);
    SEGMENT.setPixelColorXY((uint16_t)ball->x, (uint16_t)ball->y, color);
    // previousX = (uint16_t)ball->x;
    // previousY = (uint16_t)ball->y;

    SEGMENT.drawLine(0, racket_left->y, 0, racket_left->y + racket_left->size-1, color);
    SEGMENT.drawLine(vW-1, racket_right->y, vW-1, racket_right->y + racket_right->size-1, color);

    for (int i=0; i<vH; i+=2) {
      SEGMENT.setPixelColorXY(vW/2, i, color);
    }

  char tempString[2] = "";
  sprintf(tempString, "%1d%1d", ball->scoreRight, ball->scoreLeft);
  SEGMENT.drawCharacter(tempString[0], vW/2-5, -2, 5, 8, color);
  SEGMENT.drawCharacter(tempString[1], vW/2+2, -2, 5, 8, color);
