...
private:
  ...
  bool handleOffTimer()
  {
    if (m_offTimerStart > 0 && millis() - m_offTimerStart > m_switchOffDelay)
    {
      ...
      {
        if (m_lightEnought) bri=0;
        else{
        effectCurrent = 38; // aurora?
        nightlightActive = true; // con esto hace crossfade para apagarse
        }
      }
      m_offTimerStart = 0;
      m_lightActive = false;
...
public:
  ...
  void loop()
  {
    if(bri>0) // the lights are off
    {
      if (digitalRead(PIRsensorPin) == HIGH && prevState == LOW)
      { // Motion detected
        publishMqtt("ON");
        prevState = HIGH;
        m_offTimerStart = millis(); // every time move is detected reset the countdown
        if (!m_lightActive) m_lightValue = analogRead(LIGHT_PIN);
        m_lightActive = true;
        switchStrip(); 
      } 
    }
    if (digitalRead(PIRsensorPin) == LOW && prevState == HIGH)
    {  // Motion stopped
      publishMqtt("OFF");
      prevState = LOW;
    }
