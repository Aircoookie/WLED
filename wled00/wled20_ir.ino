...
void decodeIR24OLD(uint32_t code)
{
  switch (code) {
    ...
    case IR24_OLD_FLASH     : if (!applyPreset(1)) effectCurrent = FX_MODE_COLORTWINKLE;  break;
    case IR24_OLD_STROBE    : if (!applyPreset(2)) effectCurrent = FX_MODE_RAINBOW_CYCLE; break;
    case IR24_OLD_FADE      : if (!applyPreset(3)) effectCurrent = FX_MODE_BREATH;        break;
    case IR24_OLD_SMOOTH    : if (!applyPreset(4)) effectCurrent = FX_MODE_RAINBOW;       break;
    default: return;
  }
  lastValidCode = code;
  colorUpdated(2); //for notifier, IR is considered a button input
...
void decodeIR24CT(uint32_t code)
{
  switch (code) {
    ...
    case IR24_CT_MEMORY     : colorFromUint32(COLOR_NEUTRALWHITE); effectCurrent = 0;  break;
    default: return; 
  }
  lastValidCode = code;
  colorUpdated(2); //for notifier, IR is considered a button input
...
void decodeIR40(uint32_t code)
{
  switch (code) {
    ...
    case IR40_JUMP3        : if (!applyPreset(1)) effectCurrent = FX_MODE_STATIC;           break;
    case IR40_FADE3        : if (!applyPreset(2)) effectCurrent = FX_MODE_BREATH;           break;
    case IR40_FADE7        : if (!applyPreset(3)) effectCurrent = FX_MODE_FIRE_FLICKER;     break;
    case IR40_FLASH        : if (!applyPreset(4)) effectCurrent = FX_MODE_RAINBOW;          break;
  }
  lastValidCode = code;
  colorUpdated(2); //for notifier, IR is considered a button input 
}
...
void decodeIR44(uint32_t code)
{
  switch (code) {
    ...
    case IR44_WHITE       : colorFromUint32(COLOR_NEUTRALWHITE); effectCurrent = 0;        break;                              break;
    ...
    case IR44_DIY1        : if (!applyPreset(1)) effectCurrent = FX_MODE_STATIC;           break;
    case IR44_DIY2        : if (!applyPreset(2)) effectCurrent = FX_MODE_BREATH;           break;
    case IR44_DIY3        : if (!applyPreset(3)) effectCurrent = FX_MODE_FIRE_FLICKER;     break;
    case IR44_DIY4        : if (!applyPreset(4)) effectCurrent = FX_MODE_RAINBOW;          break;
    case IR44_DIY5        : if (!applyPreset(5)) effectCurrent = FX_MODE_METEOR_SMOOTH;    break;
    case IR44_DIY6        : if (!applyPreset(6)) effectCurrent = FX_MODE_RAIN;             break;
