...
void handleSettingsSet(AsyncWebServerRequest *request, byte subPage)
{
  ...
  if (subPage == 10)
  {
    ...
    String ledJson = request->arg(F("MAP"));
    uint16_t index = 0;
    int start = 2;
    while (true){
      int end = ledJson.indexOf(']',start);
      String nums = ledJson.substring(start,end);
      
      int comma = nums.indexOf(",");
      int num1 = nums.substring(0,comma).toInt();
      int num2 = nums.substring(comma+1).toInt();

      strip.insertMatrix(num1,num2,index);

      index += 1;
      start = end + 3;
      if(start >= ledJson.length()){
        break;
      }
    }
  }
