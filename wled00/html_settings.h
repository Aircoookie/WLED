...
accept=".json"> <input type="button" value="Upload" onclick="uploadFile()"><br>
</div><div id="toast"></div><a 
...
<button type="button" onclick="H()">?</button></div><button type="button" 
onclick="B()">Back</button><button type="submit">Save</button><hr></div><h2>
Sync setup</h2><h3>WLED Broadcast</h3>UDP Port: <input name="UP" type="number" 
...
target="_blank">Analog Button setup</a></div><h3>Time-controlled presets</h3>
<div style="display:inline-block"><table id="TMT"></table></div><hr><button 
