...
var d=document;function B(){window.open("/settings","_self")}function gId(n){return d.getElementById(n)}function showBrCtrlVal(){gId("minbv").innerText=d.Sf.minb.value,gId("maxbv").innerText=d.Sf.maxb.value}function adjustRange(n){n?d.Sf.minb.value=Math.min(parseInt(d.Sf.minb.value),parseInt(d.Sf.maxb.value)-1):d.Sf.maxb.value=Math.max(parseInt(d.Sf.maxb.value),parseInt(d.Sf.minb.value)+1)}function updateBrCtrl(){var n=d.Sf.autb.checked;gId("brctrl").style.opacity=n?"1":".5",d.Sf.minb.disabled=!n,d.Sf.maxb.disabled=!n,showBrCtrlVal()}function init(){RangeTouch.setup('input[type="range"]',{}),updateBrCtrl()}function GetV() {var d=document;
%CSS%%SCSS%%LCSCSS%<style type="text/css">#brctrl{transition:opacity .4s}</style></head>
<body onload="GetV(),init()"><form id="form_s" name="Sf" method="post"><button 
type="button" onclick="B()">Back</button><button type="submit">Save</button><hr>
<h2>Clock Settings</h2><h3>Brightness Control</h3><input type="checkbox" 
name="autb" onchange="updateBrCtrl()"> Automatic brightness control:<br><div 
id="brctrl" class="il">Minimum brightness: <span id="minbv"></span><br><div 
class="sliderwrap il"><input id="minb" name="minb" max="249" min="1" 
