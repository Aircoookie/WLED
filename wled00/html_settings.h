...
LED Preferences</button></form><form action="/settings/relays"><button 
type="submit">Relay Setup</button></form><form action="/settings/ui"><button 
...
// Autogenerated from wled00/data/../../usermods/GeoGab-Relays/settings_relay.htm, do not edit!!
const char PAGE_settings_relay[] PROGMEM = R"=====(<!DOCTYPE html><html><head><meta charset="utf-8"><meta name="viewport" 
content="width=500"><title>Relais Settings</title><style>
body{font-family:Verdana,sans-serif;text-align:center;background:#222;color:#fff;line-height:200%;margin:0}hr{border-color:#666}button{background:#333;color:#fff;font-family:Verdana,sans-serif;border:.3ch solid #333;display:inline-block;font-size:20px;margin:0 8px}button.ext{border-radius:5px;font-size:18px}.helpB{text-align:left;position:absolute;width:60px}input{background:#333;color:#fff;font-family:Verdana,sans-serif;font-size:large;border:.5ch solid #333}input[type=number]{width:2em}input[type=text]{width:10em}select{background:#333;color:#fff;font-family:Verdana,sans-serif;border:.5ch solid #333}td{padding:2px}.d5{width:4.5em!important}#toast{opacity:0;background-color:#555;max-width:90%;color:#fff;text-align:center;border-radius:5px;padding:16px;position:fixed;z-index:5;left:50%;transform:translateX(-50%);bottom:75px;pointer-events:none}#toast.show{opacity:1;animation:fadein .5s,fadein .5s 2.5s reverse}#toast.error{opacity:1;background-color:#b21;animation:fadein .5s,fadein .5s 2.5s reverse}.devtab{background-color:#052a33;border:1px solid;width:500px;padding:5px;margin:0 auto}input[type=checkbox]{display:none;position:absolute}input[type=checkbox]+label::before{width:15px;height:15px;border-radius:3px;border:2px solid #676864;background-color:#fff;display:inline-block;content:" ";vertical-align:middle}input[type=checkbox]:checked+label::before{box-shadow:inset 0 0 0 3px #fff;background-color:#1b801b}input::-webkit-inner-spin-button,input::-webkit-outer-spin-button{-webkit-appearance:none;margin:0}input[type=number]{-moz-appearance:textfield}
</style><script>
var resp,timeout,jsetup={},max=0,seq=0;function H(){window.open("https://github.com/Aircoookie/WLED/wiki/")}function B(){window.open("/settings","_self")}function showToast(e,t=!1){t&&(document.getElementById("connind").style.backgroundColor="#831");var s=document.getElementById("toast");s.innerHTML=e,s.className=t?"error":"show",clearTimeout(timeout),s.style.animation="none",s.style.animation=null,timeout=setTimeout((function(){s.classList.remove("show"),s.classList.remove("error")}),2900)}function GetJson(){jsetup.relays={no:0},jsetup.relays={config:1},jsetup.relays.name=[],jsetup.relays.gpio=[],jsetup.relays.sactive=[],jsetup.relays.invert=[],fetch("http://10.7.7.4/json/state").then(e=>e.json()).then(e=>{if(max=e.relays.max,jsetup.relays.no=e.relays.no,e.relays.no)for(i=0;i<jsetup.relays.no;i++)jsetup.relays.name[i]=e.relays.name[i],jsetup.relays.gpio[i]=e.relays.gpio[i],jsetup.relays.sactive[i]=e.relays.sactive[i],jsetup.relays.invert[i]=e.relays.invert[i];list()})}function list(){var e="<tr><th>Name</th><th>GPIO</th><th>Starup</th><th>Invert</th><th>Delete</th></tr>";if(jsetup.relays.no)for(i=0;i<jsetup.relays.no;i++)e+='<tr><td><input type="text" onchange="change(this,'+i+')" id="name'+i+'" value="'+jsetup.relays.name[i]+'"></td>',e+='<td><input type="number" onchange="change(this,'+i+')" min="0" max="99" id="gpio'+i+'" value="'+jsetup.relays.gpio[i]+'"></td>',e+='<td><input type="checkbox" onchange="change(this,'+i+')" id="sactive'+i+'" '+(jsetup.relays.sactive[i]?'checked="checked"':"")+'"><label for="sactive'+i+'"></label></td>',e+='<td><input type="checkbox" onchange="change(this,'+i+')" id="invert'+i+'" '+(jsetup.relays.invert[i]?'checked="checked"':"")+'"><label for="invert'+i+'"></label></td>',e+='<td><button class="ext" onclick="del('+i+')">DEL</button></td></tr>';max>jsetup.relays.no?document.getElementById("AddButton").innerHTML='<button class="ext" onclick="add();">ADD Relay</button><br>':document.getElementById("AddButton").innerHTML="",document.getElementById("ListOfRelays").innerHTML=e,document.getElementById("maxdev").innerHTML=max}function change(e,t){noexitflag=!1,jsetup.relays.name[t]=document.getElementById("name"+t).value,jsetup.relays.gpio[t]=document.getElementById("gpio"+t).value,jsetup.relays.sactive[t]=document.getElementById("sactive"+t).checked?1:0,jsetup.relays.invert[t]=document.getElementById("invert"+t).checked?1:0}function validate(){for(var e=!1,t=0;t<jsetup.relays.no;t++)document.getElementById("name"+t).setCustomValidity("");for(t=0;t<jsetup.relays.no;t++)for(var s=t+1;s<jsetup.relays.no;s++)jsetup.relays.name[t]==jsetup.relays.name[s]&&(e=!0,document.getElementById("name"+s).setCustomValidity("duplicate"));for(t=0;t<jsetup.relays.no;t++)document.getElementById("gpio"+t).setCustomValidity("");for(t=0;t<jsetup.relays.no;t++)for(s=t+1;s<jsetup.relays.no;s++)jsetup.relays.gpio[t]==jsetup.relays.gpio[s]&&(e=!0,document.getElementById("gpio"+s).setCustomValidity("duplicate"),console.error("Duplicate gpio found in Line: "+s));return e}function add(){seq++,jsetup.relays.name[jsetup.relays.no]="Relay "+seq,jsetup.relays.gpio[jsetup.relays.no]=0,jsetup.relays.sactive[jsetup.relays.no]=0,jsetup.relays.invert[jsetup.relays.no]=1,jsetup.relays.no++,list()}function del(e){jsetup.relays.name.splice(e,1),jsetup.relays.gpio.splice(e,1),jsetup.relays.sactive.splice(e,1),jsetup.relays.invert.splice(e,1),jsetup.relays.no--,list()}function save(){validate()?showToast("Duplicates in Name/GPIO are not allowed!",!0):(console.log("Sending: "+JSON.stringify(jsetup)),fetch("http://10.7.7.4/json",{method:"POST",body:JSON.stringify(jsetup),headers:{"Content-type":"application/json; charset=UTF-8"}}).then(e=>e.json()).then((function(e){console.log(e),e.success?showToast("Successfully saved!"):showToast("Error Saving Settings!",!1)})))}function GetV(){}
</script></head><body onload="GetJson()"><div class="helpB"><button 
type="button" onclick="H()">?</button></div><button type="button" onclick="B()">
Back</button><button onclick="save()">Save</button><hr><h2>Relay setup</h2><div 
id="connind"></div><div id="toast"></div><h3>Relays Definitions</h3><table 
class="devtab" id="ListOfRelays"></table><div id="AddButton"></div><br><i>
Max Devices: <span id="maxdev"></span><br><b>Name</b>
: The Name of the Relay is used for Alexa too. Please use unique names.<br><b>
GPIO</b>
: Pin number to which the relay is connected. Please refer to the notes below.
<br><b>Startup</b>: Determines if the relay is on or off on startup/reboot.<br>
<b>Invert</b>
: Most relays are active when the pin is low (active low). Invert to operate these relays correctly.
<br></i><br><h3>Important Note</h3>
Please keep in mind that some GPIO's are already occupied. Act with care in order to avoid destroying something on your hardware.
