...
var d=document,laprev=55,maxST=2,bmax=5e3,LCmax=1536,bquot=0;function H(){window.open("https://github.com/Aircoookie/WLED/wiki/Settings#led-settings")}function B(){window.open("/settings","_self")}function trySubmit(){var e=d.getElementsByTagName("input");for(i=0;i<e.length;i++){var n=e[i].name.substring(0,2);if(("L0"==n||"L1"==n||"L2"==t||"L3"==t||"L4"==t||"RL"==n||"BT"==n||"IR"==n||"AX"==n)&&""!=e[i].value&&"-1"!=e[i].value){if(d.um_p&&d.um_p.some(n=>n==parseInt(e[i].value,10)))return alert("Usermod/reserved pin conflict!"),void e[i].focus();for(j=i+1;j<e.length;j++){var t=e[j].name.substring(0,2);if(("L0"==t||"L1"==t||"L2"==t||"L3"==t||"L4"==t||"RL"==t||"BT"==t||"IR"==t||"AX"==t)&&""!=e[j].value&&e[i].value==e[j].value)return alert("Pin conflict!"),void e[j].focus()}}}if(bquot>100){var a="Too many LEDs for me to handle!";return bmax<1e4&&(a+="\n\rConsider using an ESP32."),void alert(a)}d.Sf.reportValidity()&&d.Sf.submit()}function S(){GetV(),setABL(),d.getElementById("m1").innerHTML=bmax}function enABL(){var e=d.getElementById("able").checked;d.Sf.LA.value=e?laprev:0,d.getElementById("abl").style.display=e?"inline":"none",d.getElementById("psu2").style.display=e?"inline":"none",d.Sf.LA.value>0&&setABL()}function enLA(){var e=d.Sf.LAsel.value;d.Sf.LA.value=e,d.getElementById("LAdis").style.display=50==e?"inline":"none",UI()}function setABL(){switch(d.getElementById("able").checked=!0,d.Sf.LAsel.value=50,parseInt(d.Sf.LA.value)){case 0:d.getElementById("able").checked=!1,enABL();break;case 30:d.Sf.LAsel.value=30;break;case 35:d.Sf.LAsel.value=35;break;case 55:d.Sf.LAsel.value=55;break;case 255:d.Sf.LAsel.value=255;break;default:d.getElementById("LAdis").style.display="inline"}UI()}function getMem(e,n,t){return e<32?bmax<1e4&&3==t?e>29?20*n:15*n:bmax>1e4?e>29?8*n:6*n:e>29?4*n:3*n:e>31&&e<48?5:44==e||45==e?4*n:3*n}function UI(e=!1){var n=!1,t=0;d.getElementById("ampwarning").style.display=d.Sf.MA.value>7200?"inline":"none",255==d.Sf.LA.value?laprev=12:d.Sf.LA.value>0&&(laprev=d.Sf.LA.value);var a=d.getElementsByTagName("select");for(c=0;c<a.length;c++)if("LT"==a[c].name.substring(0,2)){L=a[c].name.substring(2);var i=parseInt(a[c].value,10);d.getElementById("p0d"+L).innerHTML=i>49?"Data:":i>41?"Pins:":"Pin:",d.getElementById("p1d"+L).innerHTML=i>49?"Clk:":"";var l=d.getElementsByName("L1"+L)[0];for(t+=getMem(i,d.getElementsByName("LC"+L)[0].value,d.getElementsByName("L0"+L)[0].value),p=1;p<5;p++){(l=d.getElementsByName("L"+p+L)[0])&&(i>49&&1==p||i>41&&i<50&&p+40<i?(l.style.display="inline",l.required=!0):(l.style.display="none",l.required=!1,l.value=""))}d.getElementById("ls"+L).readOnly=!(i>31&&i<48),d.getElementById("LC").readOnly=!(i>31&&i<48),e&&(d.getElementById("ew"+L).checked=30==i||31==i||44==i||45==i,d.getElementById("ls"+L).value=L+1),d.getElementById("ew"+L).onclick=i>31&&i<48?function(){return!1}:function(){},n|=d.getElementById("ew"+L).checked,d.getElementById("dig"+L).style.display=i>31&&i<48?"none":"inline",d.getElementById("psd"+L).innerHTML=i>31&&i<48?"Index:":"Start:"}var o=d.querySelectorAll(".wc"),s=o.length;for(c=0;c<s;c++)o[c].style.display=n?"inline":"none";var r=d.getElementsByTagName("input"),u=0,m=0;for(c=0;c<r.length;c++){var v=r[c].name.substring(0,2);if("LC"!=v||"LC"===r[c].name){if("L0"==v||"L1"==v)d.getElementsByName("LC"+r[c].name.substring(2))[0].max=LCmax;if(("L0"==v||"L1"==v||"L2"==v||"L3"==v||"L4"==v||"RL"==v||"BT"==v||"IR"==v||"AX"==v)&&""!=r[c].value&&"-1"!=r[c].value){if(d.um_p&&d.um_p.some(e=>e==parseInt(r[c].value,10))){alert("Usermod/reserved pin conflict!"),r[c].focus();continue}for(j=0;j<r.length;j++)if(c!=j){var y=r[j].name.substring(0,2);if(("L0"==y||"L1"==y||"L2"==y||"L3"==y||"L4"==y||"RL"==y||"BT"==y||"IR"==y||"AX"==y)&&""!=r[j].value&&r[c].value==r[j].value){alert("Pin conflict!"),r[j].value="",r[j].focus();break}}}}else{var L=r[c].name.substring(2),g=parseInt(r[c].value,10);d.getElementById("ls"+L).readOnly&&(d.getElementById("ls"+L).value=u),g&&(u+=g,g>m&&(m=g))}}if(d.getElementById("LC").readOnly&&(d.getElementsByName("LC")[0].value=u),d.activeElement==d.getElementsByName("LC")[0]){var c=d.getElementsByClassName("iST").length;1==c&&(d.getElementsByName("LC0")[0].value=d.getElementsByName("LC")[0].value)}d.getElementById("m0").innerHTML=t,bquot=t/bmax*100,d.getElementById("dbar").style.background=`linear-gradient(90deg, ${bquot>60?bquot>90?"red":"orange":"#ccc"} 0 ${bquot}%%, #444 ${bquot}%% 100%%)`,d.getElementById("ledwarning").style.display=u>LCmax||m>800||bquot>80?"inline":"none",d.getElementById("ledwarning").style.color=u>LCmax||m>LCmax||bquot>100?"red":"orange",d.getElementById("wreason").innerHTML=bquot>80?"80% of max. LED memory"+(bquot>100?` (<b>WARNING: Using over ${bmax}B!</b>)`:""):"800 LEDs per pin";var B=Math.ceil((100+u*laprev)/500)/2;B=B>5?Math.ceil(B):B;a="";var f=30==d.Sf.LAsel.value,b=255==d.Sf.LAsel.value;B<1.02&&!f&&!b?a="ESP 5V pin with 1A USB supply":(a+=f?"12V ":b?"WS2815 12V ":"5V ",a+=B,a+="A supply connected to LEDs");var E=Math.ceil((100+u*laprev)/1500)/2,I="(for most effects, ~";I+=E=E>5?Math.ceil(E):E,I+="A is enough)<br>",d.getElementById("psu").innerHTML=a,d.getElementById("psu2").innerHTML=b?"":I}function addLEDs(e){if(e>1)return maxST=e,void(d.getElementById("+").style.display="inline");var n=d.getElementsByClassName("iST"),t=n.length;if(!(1==e&&t>=maxST||-1==e&&0==t)){var a=d.getElementById("mLC");if(1==e){var i=`<div class="iST">\n${t>0?'<hr style="width:260px">':""}\n${t+1}:\n<select name="LT${t}" onchange="UI(true)">\n<option value="22">WS281x</option>\n<option value="30">SK6812 RGBW</option>\n<option value="31">TM1814</option>\n<option value="24">400kHz</option>\n<option value="50">WS2801</option>\n<option value="51">APA102</option>\n<option value="52">LPD8806</option>\n<option value="53">P9813</option>\n<option value="41">PWM White</option>\n<option value="42">PWM WWCW</option>\n<option value="43">PWM RGB</option>\n<option value="44">PWM RGBW</option>\n<option value="45">PWM RGBWC</option>\n</select>&nbsp;\nColor Order:\n<select name="CO${t}">\n<option value="0">GRB</option>\n<option value="1">RGB</option>\n<option value="2">BRG</option>\n<option value="3">RBG</option>\n<option value="4">BGR</option>\n<option value="5">GBR</option>\n</select><br>\n<span id="p0d${t}">Pin:</span> <input type="number" name="L0${t}" min="0" max="40" required style="width:35px" onchange="UI()"/>\n<span id="p1d${t}">Clock:</span> <input type="number" name="L1${t}" min="0" max="40" style="width:35px" onchange="UI()"/>\n<span id="p2d${t}"></span><input type="number" name="L2${t}" min="0" max="40" style="width:35px" onchange="UI()"/>\n<span id="p3d${t}"></span><input type="number" name="L3${t}" min="0" max="40" style="width:35px" onchange="UI()"/>\n<span id="p4d${t}"></span><input type="number" name="L4${t}" min="0" max="40" style="width:35px" onchange="UI()"/>\n<br>\n<span id="psd${t}">Start:</span> <input type="number" name="LS${t}" id="ls${t}" min="0" max="8191" value="0" required />&nbsp;\n<div id="dig${t}" style="display:inline">\nCount: <input type="number" name="LC${t}" min="0" max="2048" value="1" required oninput="UI()" /><br>\nReverse (rotated 180°): <input type="checkbox" name="CV${t}">\n</div>\n&nbsp;RGBW: <input id="ew${t}" type="checkbox" name="EW${t}"><br>\n</div>`;a.insertAdjacentHTML("beforeend",i)}-1==e&&(n[--t].remove(),--t),d.getElementById("+").style.display=t<maxST-1?"inline":"none",d.getElementById("-").style.display=t>0?"inline":"none",UI()}}function GetV() {var d=document;
...
style="width:260px">Relay pin: <input type="number" min="-1" max="40" name="RL" 
...
<input type="number" min="-1" max="40" name="AX" onchange="UI()"><h3>Defaults
</h3>Turn LEDs on after power up/reset: <input type="checkbox" name="BO"><br>
