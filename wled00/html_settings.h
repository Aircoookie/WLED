...
</form><form action="/settings/ledclock"><button type="submit">Clock Settings
</button></form><form action="/settings/wifi"><button type="submit">WiFi Setup
...
type="submit">Time & Macros</button></form><form action="/settings/sec"><button 
...
</div><div style="display:none"><h3>Clock</h3>Analog Clock overlay: <input 
...
type="number" min="0" max="59" required></nowrap><br></div><h3>Macro presets
...
type="number" min="0" max="250" required><br><div style="display:none"><h3>
Button actions</h3><table style="margin:0 auto" id="macros"><thead><tr><td>push
<br>switch</td><td>short<br>on-&gt;off</td><td>long<br>off-&gt;on</td><td>double
<br>N/A</td></tr></thead><tbody></tbody></table><a 
href="https://kno.wled.ge/features/macros/#analog-button" target="_blank">
Analog Button setup</a></div><h3>Time-controlled presets</h3><div 
...
href="https://github.com/Aircoookie/WLED/blob/master/LICENSE" target="_blank">
MIT license</a></i><br><br>Icons used in this version are from <a 
href="https://icons8.com/line-awesome" target="_blank">LINE AWESOME</a><br><br>
Server message: <span class="sip">Response error!</span><hr><div id="toast">
</div><button type="button" onclick="B()">Back</button><button type="submit">
Save & Reboot</button></form><iframe name="download-frame" style="display:none">
</iframe></body></html>)=====";
...
const char PAGE_settings_ledclock[] PROGMEM = R"=====(<!DOCTYPE html><html lang="en"><head><meta name="viewport" content="width=500">
<meta charset="utf-8"><title>Clock Settings</title>%RTJS%<script>
var lcBeeps=%LCBEEPSJS%,d=document;function B(){window.open("/settings","_self")}function gId(e){return d.getElementById(e)}function showBrCtrlVal(){gId("minbv").innerText=d.Sf.minb.value,gId("maxbv").innerText=d.Sf.maxb.value}function adjustRange(e){e?d.Sf.minb.value=Math.min(parseInt(d.Sf.minb.value),parseInt(d.Sf.maxb.value)-1):d.Sf.maxb.value=Math.max(parseInt(d.Sf.maxb.value),parseInt(d.Sf.minb.value)+1)}function updateBrCtrl(){var e=d.Sf.autb.checked;gId("brctrl").style.opacity=e?"1":".5",d.Sf.minb.disabled=!e,d.Sf.maxb.disabled=!e,showBrCtrlVal()}function beep(e){fetch("/json/state",{method:"post",headers:{"Content-type":"application/json; charset=UTF-8"},body:JSON.stringify({ledclock:{cmd:99,beep:e}})}).catch(e=>console.log(e))}function init(){var e=d.querySelectorAll(".beeptable select");for(var t in e.forEach(e=>e.insertAdjacentHTML("beforeend",'<option value="255">None</option>')),lcBeeps)e.forEach(e=>e.insertAdjacentHTML("beforeend",`<option value="${t}">${lcBeeps[t]}</option>`));e.forEach(e=>e.addEventListener("input",e=>{var t=parseInt(e.target.value);255!=t&&beep(t)})),RangeTouch.setup('input[type="range"]',{}),updateBrCtrl()}function GetV() {var d=document;
%CSS%%SCSS%%LCSCSS%<style type="text/css">
#brctrl{transition:opacity .4s}.beeptable{display:grid;grid-template-columns:max-content max-content;justify-items:end;row-gap:.2rem;column-gap:.3rem;justify-content:center}
</style></head><body onload="init(),GetV()"><form id="form_s" name="Sf" 
method="post"><button type="button" onclick="B()">Back</button><button 
type="submit">Save</button><hr><h2>LED Clock Settings</h2><h3>Brightness Control
</h3><input type="checkbox" name="autb" onchange="updateBrCtrl()">
 Automatic brightness control:<br><div id="brctrl" class="il">
Minimum brightness: <span id="minbv"></span><br><div class="sliderwrap il">
<input id="minb" name="minb" max="249" min="1" type="range" 
oninput="adjustRange(!1),showBrCtrlVal()"><div class="sliderdisplay"></div>
</div><br>Maximum brightness: <span id="maxbv"></span><br><div 
class="sliderwrap il"><input id="maxb" name="maxb" max="255" min="2" 
type="range" oninput="adjustRange(!0),showBrCtrlVal()"><div 
...
value="1">Off</option><option value="2">Blink</option></select><h3>Beeps</h3>
<input type="checkbox" name="bmute"> Mute all beeps<h4 id="clock">
Beeps on events:</h4><div class="beeptable"><span>Startup:</span><select 
name="bstartup"></select> <span>Wi-Fi connected:</span><select name="bwifi">
</select></div><h4 id="clock">Clock beeps on:</h4><div class="beeptable"><span>
Each minute:</span><select name="clbm"></select> <span>Each hour:</span><select 
name="clbh"></select></div><h4 id="timer">Timer beeps on:</h4><div 
class="beeptable"><span>Time setting:</span><select name="tmbset"></select> 
<span>Start:</span><select name="tmbstart"></select> <span>Pause:</span><select 
name="tmbpause"></select> <span>Resume:</span><select name="tmbresume"></select>
 <span>Reset:</span><select name="tmbreset"></select> <span>Time addition:
</span><select name="tmbtinc"></select> <span>Each hour:</span><select 
name="tmbh"></select> <span>Each minute:</span><select name="tmbm"></select> 
<span>Secs. of last min.:</span><select name="tmbs"></select> <span>Timeout:
</span><select name="tmbto"></select></div><h4 id="stopwatch">
Stopwatch beeps on:</h4><div class="beeptable"><span>Start:</span><select 
name="swbstart"></select> <span>Pause:</span><select name="swbpause"></select> 
<span>Resume:</span><select name="swbresume"></select> <span>Reset:</span>
<select name="swbreset"></select> <span>Secs. of first min.:</span><select 
name="swbs"></select> <span>Each minute:</span><select name="swbm"></select> 
<span>Each hour:</span><select name="swbh"></select> <span>Lap time:</span>
<select name="swblapt"></select></div><br><hr><div id="toast"></div><button 
...
const char PAGE_ledClockSettingsCss[] PROGMEM = R"=====(<style>.sliderdisplay{content:'';position:absolute;top:12.5px;bottom:12.5px;left:13px;right:13px;background:#444;border-radius:17px;pointer-events:none;z-index:-1}.sliderbubble{width:36px;line-height:24px;background:#333;position:absolute;transform:translateX(-50%%);border-radius:12px;margin-left:12px;margin-top:3px;padding:0;display:inline}input[type=range]{-webkit-appearance:none;width:220px;padding:0;margin:0 10px 0 10px;background-color:transparent;cursor:pointer;border:0}input[type=range]:focus{outline:0}input[type=range]::-webkit-slider-runnable-track{width:100%%;height:30px;cursor:pointer;background:0 0}input[type=range]::-webkit-slider-thumb{height:16px;width:16px;border-radius:17px;background:#fff;cursor:pointer;-webkit-appearance:none;margin-top:7px}input[type=range]::-moz-range-track{width:100%%;height:30px;background-color:rgba(0,0,0,0)}input[type=range]::-moz-range-thumb{border:0 solid transparent;height:16px;width:16px;border-radius:17px;background:#fff;transform:translateY(7px)}input[type=range]:active+.sliderbubble{display:inline;transform:translateX(-50%%)}.sliderwrap{height:30px;width:240px;position:relative}.il{display:inline-block;vertical-align:middle}</style>)=====";
