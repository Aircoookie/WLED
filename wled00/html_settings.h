...
var d=document,laprev=55,maxST=1;function H(){window.open("https://github.com/Aircoookie/WLED/wiki/Settings#led-settings")}function B(){window.open("/settings","_self")}function S(){GetV(),setABL()}function enABL(){var e=d.getElementById("able").checked;d.Sf.LA.value=e?laprev:0,d.getElementById("abl").style.display=e?"inline":"none",d.getElementById("psu2").style.display=e?"inline":"none",d.Sf.LA.value>0&&setABL()}function enLA(){var e=d.Sf.LAsel.value;d.Sf.LA.value=e,d.getElementById("LAdis").style.display=50==e?"inline":"none",UI()}function setABL(){switch(d.getElementById("able").checked=!0,d.Sf.LAsel.value=50,parseInt(d.Sf.LA.value)){case 0:d.getElementById("able").checked=!1,enABL();break;case 30:d.Sf.LAsel.value=30;break;case 35:d.Sf.LAsel.value=35;break;case 55:d.Sf.LAsel.value=55;break;case 255:d.Sf.LAsel.value=255;break;default:d.getElementById("LAdis").style.display="inline"}UI()}function UI(){var e=!1;d.getElementById("ampwarning").style.display=d.Sf.MA.value>7200?"inline":"none",255==d.Sf.LA.value?laprev=12:d.Sf.LA.value>0&&(laprev=d.Sf.LA.value);var t=d.getElementsByTagName("select");for(s=0;s<t.length;s++)if("LT"==t[s].name.substring(0,2)){n=t[s].name.substring(2);var a=t[s].value;d.getElementById("p0d"+n).innerHTML=a>49?"Data pin:":a>41?"Pins:":"Pin:",d.getElementById("p1d"+n).innerHTML=a>49?"Clk:":"";var i=d.getElementsByName("L1"+n)[0];for(p=1;p<5;p++){(i=d.getElementsByName("L"+p+n)[0])&&(a>49&&1==p||a>41&&a<50&&p+40<a?(i.style.display="inline",i.required=!0):(i.style.display="none",i.required=!1,i.value=""))}30!=a&&31!=a&&44!=a&&45!=a||(e=!0)}var l=d.querySelectorAll(".wc"),o=l.length;for(s=0;s<o;s++)l[s].style.display=e?"inline":"none";if(d.activeElement==d.getElementsByName("LC")[0]){var s=d.getElementsByClassName("iST").length;1==s&&(d.getElementsByName("LC0")[0].value=d.getElementsByName("LC")[0].value)}var u=d.getElementsByTagName("input"),m=0;for(s=0;s<u.length;s++){var r=u[s].name.substring(0,2);if("LC"!=r||"LC"==u[s].name){if("L0"==r){var v=d.getElementsByName("LC"+u[s].name.substring(2))[0];3==u[s].value?v.max=500:v.max=1500}if(("L0"==r||"L1"==r||"RL"==r||"BT"==r||"IR"==r||"AX"==r)&&""!=u[s].value&&"-1"!=u[s].value){if(d.um_p&&d.um_p.some(e=>e==parseInt(u[s].value,10))){alert("Usermod pin clash!"),u[s].value="",u[s].focus();continue}for(j=0;j<u.length;j++)if(s!=j){var y=u[j].name.substring(0,2);if(("L0"==y||"L1"==y||"RL"==y||"BT"==y||"IR"==y||"AX"==y)&&""!=u[j].value&&u[s].value==u[j].value){alert("Pin clash!"),u[s].value="",u[s].focus();break}}}}else{var L=parseInt(u[s].value,10);L&&(m+=L)}}d.getElementById("ledwarning").style.display=m>1e3?"inline":"none";var c=Math.ceil((100+m*laprev)/500)/2;c=c>5?Math.ceil(c):c;t="";var f=30==d.Sf.LAsel.value,g=255==d.Sf.LAsel.value;c<1.02&&!f&&!g?t="ESP 5V pin with 1A USB supply":(t+=f?"12V ":g?"WS2815 12V ":"5V ",t+=c,t+="A supply connected to LEDs");var B=Math.ceil((100+m*laprev)/1500)/2,A="(for most effects, ~";A+=B=B>5?Math.ceil(B):B,A+="A is enough)<br>",d.getElementById("psu").innerHTML=t,d.getElementById("psu2").innerHTML=g?"":A}function addLEDs(e){if(e>1)return maxST=e,void(d.getElementById("+").style.display="inline");var n=d.getElementsByClassName("iST"),t=n.length;if(!(1==e&&t>=maxST||-1==e&&0==t)){var a=d.getElementById("mLC");if(1==e){var i=`<div class="iST">\n          ${t>0?'<hr style="width:260px">':""}\n          ${t+1}:\n          <select name="LT${t}" onchange="UI()">\n            <option value="22">WS281x</option>\n            <option value="30">SK6812 RGBW</option>\n            <option value="31">TM1814</option>\n            <option value="24">400kHz</option>\n            <option value="50">WS2801</option>\n            <option value="51">APA102</option>\n            <option value="52">LPD8806</option>\n            <option value="53">P9813</option>\n            <option value="41">PWM White</option>\n            <option value="42">PWM WWCW</option>\n            <option value="43">PWM RGB</option>\n            <option value="44">PWM RGBW</option>\n            <option value="45">PWM RGBWC</option>\n          </select>&nbsp;\n          Color Order:\n          <select name="CO${t}">\n            <option value="0">GRB</option>\n            <option value="1">RGB</option>\n            <option value="2">BRG</option>\n            <option value="3">RBG</option>\n            <option value="4">BGR</option>\n            <option value="5">GBR</option>\n          </select><br>\n          <span id="p0d${t}">Pin:</span> <input type="number" name="L0${t}" min="0" max="40" required style="width:35px"/>\n          <span id="p1d${t}">Clock:</span> <input type="number" name="L1${t}" min="0" max="40" style="width:35px"/>\n          <span id="p2d${t}"></span><input type="number" name="L2${t}" min="0" max="40" style="width:35px"/>\n          <span id="p3d${t}"></span><input type="number" name="L3${t}" min="0" max="40" style="width:35px"/>\n          <span id="p4d${t}"></span><input type="number" name="L4${t}" min="0" max="40" style="width:35px"/>\n          <br>\n          Start: <input type="number" name="LS${t}" min="0" max="1500" required oninput="UI()" />&nbsp;\n          Count: <input type="number" name="LC${t}" min="0" max="1500" required oninput="UI()" /><br>\n          Reverse: <input type="checkbox" name="CV${t}"><br>\n        </div>`;a.insertAdjacentHTML("beforeend",i)}-1==e&&(n[--t].remove(),--t),d.getElementById("+").style.display=t<maxST-1?"inline":"none",d.getElementById("-").style.display=t>0?"inline":"none",UI()}}function GetV() {var d=document;
...
Total LED count: <input name="LC" type="number" min="1" max="1500" 
...
<br>Relay pin: <input type="number" min="-1" max="40" name="RL" onchange="UI()">
