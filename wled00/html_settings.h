...
var d=document,laprev=55,maxST=1,bmax=5e3,bquot=0;function H(){window.open("https://github.com/Aircoookie/WLED/wiki/Settings#led-settings")}function B(){window.open("/settings","_self")}function trySubmit(){var e=d.getElementsByTagName("input");for(i=0;i<e.length;i++){var n=e[i].name.substring(0,2);if(("L0"==n||"L1"==n||"RL"==n||"BT"==n||"IR"==n||"AX"==n)&&""!=e[i].value&&"-1"!=e[i].value){if(d.um_p&&d.um_p.some(n=>n==parseInt(e[i].value,10)))return alert("Usermod pin conflict!"),void e[i].focus();for(j=i+1;j<e.length;j++){var t=e[j].name.substring(0,2);if(("L0"==t||"L1"==t||"RL"==t||"BT"==t||"IR"==t||"AX"==t)&&""!=e[j].value&&e[i].value==e[j].value)return alert("Pin conflict!"),void e[i].focus()}}}if(bquot>100){var a="Too many LEDs for me to handle!";return bmax<1e4&&(a+=" Consider using an ESP32."),void alert(a)}d.Sf.reportValidity()&&d.Sf.submit()}function S(){GetV(),setABL(),maxST>4&&(bmax=64e3),d.getElementById("m1").innerHTML=bmax}function enABL(){var e=d.getElementById("able").checked;d.Sf.LA.value=e?laprev:0,d.getElementById("abl").style.display=e?"inline":"none",d.getElementById("psu2").style.display=e?"inline":"none",d.Sf.LA.value>0&&setABL()}function enLA(){var e=d.Sf.LAsel.value;d.Sf.LA.value=e,d.getElementById("LAdis").style.display=50==e?"inline":"none",UI()}function setABL(){switch(d.getElementById("able").checked=!0,d.Sf.LAsel.value=50,parseInt(d.Sf.LA.value)){case 0:d.getElementById("able").checked=!1,enABL();break;case 30:d.Sf.LAsel.value=30;break;case 35:d.Sf.LAsel.value=35;break;case 55:d.Sf.LAsel.value=55;break;case 255:d.Sf.LAsel.value=255;break;default:d.getElementById("LAdis").style.display="inline"}d.getElementById("wreason2").innerText=d.LDmax?d.LDmax:"500",UI()}function getMem(e,n,t){return e<32?bmax<1e4&&3==t?e>29?20*n:15*n:bmax>1e4?e>29?8*n:6*n:e>29?4*n:3*n:e>31&&e<48?5:44==e||45==e?4*n:3*n}function UI(e=!1){var t=!1,a=0,i=!1;d.getElementById("ampwarning").style.display=d.Sf.MA.value>7200?"inline":"none",255==d.Sf.LA.value?laprev=12:d.Sf.LA.value>0&&(laprev=d.Sf.LA.value);var l=d.getElementsByTagName("select");for(f=0;f<l.length;f++)if("LT"==l[f].name.substring(0,2)){n=l[f].name.substring(2);var o=parseInt(l[f].value,10);d.getElementById("p0d"+n).innerHTML=o>49?"Data pin:":o>41?"Pins:":"Pin:",d.getElementById("p1d"+n).innerHTML=o>49?"Clk:":"";var s=d.getElementsByName("L1"+n)[0];for(a+=getMem(o,d.getElementsByName("LC"+n)[0].value,d.getElementsByName("L0"+n)[0].value),p=1;p<5;p++){(s=d.getElementsByName("L"+p+n)[0])&&(o>49&&1==p||o>41&&o<50&&p+40<o?(s.style.display="inline",s.required=!0):(s.style.display="none",s.required=!1,s.value=""))}d.getElementById("ls"+n).readOnly=!(o>31&&o<48),d.getElementById("LC").readOnly=!(o>31&&o<48),e&&(d.getElementById("ew"+n).checked=30==o||31==o||44==o||45==o),t|=d.getElementById("ew"+n).checked,d.getElementById("dig"+n).style.display=o>31&&o<48?"none":"inline",d.getElementById("psd"+n).innerHTML=o>31&&o<48?"Index:":"Start:"}var m=d.querySelectorAll(".wc"),r=m.length;for(f=0;f<r;f++)m[f].style.display=t?"inline":"none";var u=d.getElementsByTagName("input"),v=0,y=0;for(f=0;f<u.length;f++){var L=u[f].name.substring(0,2);if("LC"!=L||"LC"===u[f].name){if("L0"==L||"L1"==L){var g=d.getElementsByName("LC"+u[f].name.substring(2))[0];i|=3==u[f].value,d.LDmax&&i?g.max=d.LDmax:g.max=d.LCmax}if(("L0"==L||"L1"==L||"L2"==L||"L3"==L||"L4"==L||"RL"==L||"BT"==L||"IR"==L||"AX"==L)&&""!=u[f].value&&"-1"!=u[f].value){if(d.um_p&&d.um_p.some(e=>e==parseInt(u[f].value,10))){alert("Usermod/reserved pin clash!"),u[f].value="",u[f].focus();continue}for(j=0;j<u.length;j++)if(f!=j){var c=u[j].name.substring(0,2);if(("L0"==c||"L1"==c||"L2"==c||"L3"==c||"L4"==c||"RL"==c||"BT"==c||"IR"==c||"AX"==c)&&""!=u[j].value&&u[f].value==u[j].value){alert("Pin clash!"),u[j].value="",u[j].focus();break}}}}else{var B=parseInt(u[f].value,10);d.getElementById("ls"+n).readOnly&&(d.getElementById("ls"+n).value=v),B&&(v+=B,B>y&&(y=B))}}if(d.getElementById("LC").readOnly&&(d.getElementsByName("LC")[0].value=v),d.activeElement==d.getElementsByName("LC")[0]){var f=d.getElementsByClassName("iST").length;1==f&&(d.getElementsByName("LC0")[0].value=d.getElementsByName("LC")[0].value)}d.getElementById("m0").innerHTML=a,bquot=a/bmax*100,d.getElementById("dbar").style.background=`linear-gradient(90deg, ${bquot>60?bquot>90?"red":"orange":"#ccc"} 0 ${bquot}%%, #444 ${bquot}%% 100%%)`,d.getElementById("ledwarning").style.display=v>d.LCmax||y>800||bquot>80?"inline":"none",d.getElementById("ledwarning2").style.display=d.LDmax&&i&&(v>d.LDmax||y>d.LDmax)?"inline":"none",d.getElementById("wreason").innerHTML=bquot>80?"than 60%% of max. LED memory":"800 LEDs per pin";var b=Math.ceil((100+v*laprev)/500)/2;b=b>5?Math.ceil(b):b;l="";var E=30==d.Sf.LAsel.value,I=255==d.Sf.LAsel.value;b<1.02&&!E&&!I?l="ESP 5V pin with 1A USB supply":(l+=E?"12V ":I?"WS2815 12V ":"5V ",l+=b,l+="A supply connected to LEDs");var h=Math.ceil((100+v*laprev)/1500)/2,x="(for most effects, ~";x+=h=h>5?Math.ceil(h):h,x+="A is enough)<br>",d.getElementById("psu").innerHTML=l,d.getElementById("psu2").innerHTML=I?"":x}function addLEDs(e){if(e>1)return maxST=e,void(d.getElementById("+").style.display="inline");var n=d.getElementsByClassName("iST"),t=n.length;if(!(1==e&&t>=maxST||-1==e&&0==t)){var a=d.getElementById("mLC");if(1==e){var i='<div class="iST">'+t>0?'<hr style="width:260px">':""+t+"1:"+`<select name="LT${t}" onchange="UI(true)"><option value="22">WS281x</option><option value="30">SK6812 RGBW</option><option value="31">TM1814</option><option value="24">400kHz</option><option value="50">WS2801</option><option value="51">APA102</option><option value="52">LPD8806</option><option value="53">P9813</option><option value="41">PWM White</option><option value="42">PWM WWCW</option><option value="43">PWM RGB</option><option value="44">PWM RGBW</option><option value="45">PWM RGBWC</option></select>&nbsp;Color Order:`+`<select name="CO${t}"><option value="0">GRB</option><option value="1">RGB</option><option value="2">BRG</option><option value="3">RBG</option><option value="4">BGR</option><option value="5">GBR</option></select>&nbsp;`+`RGBW: <input id="ew${t}" type="checkbox" name="EW${t}"><br>`+`<span id="p0d${t}">Pin: </span><input type="number" name="L0${t}" min="0" max="40" required style="width:35px" onchange="UI()"/>`+`<span id="p1d${t}">Clock: </span><input type="number" name="L1${t}" min="0" max="40" style="width:35px" onchange="UI()"/>`+`<span id="p2d${t}"></span><input type="number" name="L2${t}" min="0" max="40" style="width:35px" onchange="UI()"/>`+`<span id="p3d${t}"></span><input type="number" name="L3${t}" min="0" max="40" style="width:35px" onchange="UI()"/>`+`<span id="p4d${t}"></span><input type="number" name="L4${t}" min="0" max="40" style="width:35px" onchange="UI()"/><br>`+`<span id="psd${t}">Start:</span> <input type="number" name="LS${t}" id="ls${t}" min="0" max="8191" value="0" required />&nbsp;`+`<div id="dig${t}" style="display:inline">`+`Count: <input type="number" name="LC${t}" min="0" max="2048" value="1" required oninput="UI()" /><br>`+`Reverse: <input type="checkbox" name="CV${t}"></div><br></div>`;a.insertAdjacentHTML("beforeend",i)}-1==e&&(n[--t].remove(),--t),d.getElementById("+").style.display=t<maxST-1?"inline":"none",d.getElementById("-").style.display=t>0?"inline":"none",UI()}}function GetV() {var d=document;
...
type="number" min="1" max="8192" oninput="UI()" size="4" required 
...
id="wreason">800 LEDs per pin</span> for the best experience!<br></div><div 
id="ledwarning2" style="color:#ff4500;display:none">
&#9888; Pin 3 has memory restrictions.<br>Use less than <span id="wreason2">500
</span> LEDs in total to avoid memory issues!<br></div><hr style="width:260px">
