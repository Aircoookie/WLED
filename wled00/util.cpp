...
bool oappend(const char *txt)
{
  ...
  if ((obuf == nullptr) || (olen + len >= SETTINGS_STACK_BUF_SIZE))
  { // sanity checks
#ifdef WLED_DEBUG
  ...
  if (mode < strip.getModeCount())
  {
    ...
    if (lineBuffer.length() > 0)
    {
          ...
          for (size_t i = 0; i <= slider; i++)
          {
            ...
            if (i == slider)
            {
              ...
              if (names.charAt(nameBegin) == '!')
              {
                ...
                {
                case 0:
                  tmpstr = PSTR("FX Speed");
                  break;
                case 1:
                  tmpstr = PSTR("FX Intensity");
                  break;
                case 2:
                  tmpstr = PSTR("FX Custom 1");
                  break;
                case 3:
                  tmpstr = PSTR("FX Custom 2");
                  break;
                case 4:
                  tmpstr = PSTR("FX Custom 3");
                  break;
                default:
                  tmpstr = PSTR("FX Custom");
                  break;
                }
              ...
              {
                if (nameEnd < 0)
                  tmpstr = names.substring(nameBegin).c_str(); // did not find ",", last name?
                else
                  tmpstr = names.substring(nameBegin, nameEnd).c_str();
      ...
      else
      {
        ...
        {
        case 0:
          strncpy_P(dest, PSTR("FX Speed"), maxLen);
          break;
        case 1:
          strncpy_P(dest, PSTR("FX Intensity"), maxLen);
          break;
        }
        ...
        if (readObjectFromFile(fileName, nullptr, &doc))
        {
          ...
          if (!doc["n"].isNull())
          {
            ...
            if (len > 0 && len < 33)
            {
              ...
              if (ledmapNames[i - 1])
                strlcpy(ledmapNames[i - 1], name, 33);
            }
          }
          if (!ledmapNames[i - 1])
          {
            ...
            ledmapNames[i - 1] = new char[len + 1];
            if (ledmapNames[i - 1])
              strlcpy(ledmapNames[i - 1], tmp, 33);
          }
        }
