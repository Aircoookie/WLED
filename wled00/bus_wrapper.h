...
//      case I_32_BB_400_3: delete (static_cast<B_32_BB_400_3*>(busPtr)); break;
  ...
  static uint8_t getI(uint8_t busType, uint8_t* pins, uint8_t num = 0) {
    ...
    if (IS_2PIN(busType)) { //SPI LED chips
      ...
      #if defined(CONFIG_IDF_TARGET_ESP32C3)    //ESP32 C3 only has 2 RMT channels, I2S not supported, BB not allowed
      if (num > 1) return I_NONE;
      #elif defined(CONFIG_IDF_TARGET_ESP32S2)  //ESP32 S2 only has 4 RMT channels, 1 I2S supported, BB not allowed
      if (num > 5) return I_NONE;
      if (num > 4) offset = 1;
      #elif defined(CONFIG_IDF_TARGET_ESP32S3)  //ESP32 S3 only has 4 RMT channels, I2S not supported, BB not allowed
      if (num > 3) return I_NONE;
      #else
      if (num > 9) return I_NONE;               // ESP32, 8 RMT channels, 2 I2S channels, BB not allowed
