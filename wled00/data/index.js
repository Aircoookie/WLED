...
function handleVisibilityChange() {
    if (!d.hidden && new Date() - lastUpdate > 3000) requestJson();
}
...
function isRgbBlack(a) {
    return (parseInt(a.r) == 0 && parseInt(a.g) == 0 && parseInt(a.b) == 0);
}
...
function rgbBri(a) {
    return 0.2126 * parseInt(a.r) + 0.7152 * parseInt(a.g) + 0.0722 * parseInt(a.b);
}
...
function presetError(empty) {
    ...
    if (empty) cn += `You have no presets yet!`; else cn += `Sorry, there was an issue loading your presets!`;
...
function populateSegments(s) {
    ...
    for (var inst of (s.seg || [])) {
        ...
        let segp = `<div id="segp${i}" class="sbs">` + `<i class="icons slider-icon pwr ${inst.on ? "act" : ""}" id="seg${i}pwr" onclick="setSegPwr(${i})">&#xe08f;</i>` + `<div class="sliderwrap il">` + `<input id="seg${i}bri" class="noslide" onchange="setSegBri(${i})" oninput="updateTrail(this)" max="255" min="1" type="range" value="${inst.bri}" />` + `<div class="sliderdisplay"></div>` + `</div>` + `</div>`;
        let staX = inst.start;
        let stoX = inst.stop;
...
function populateEffects() {
    ...
    for (let i = 0; i < effects.length; i++) {
        effects[i] = {
            id: effects[i][0], name: effects[i][1]
        };
    }
...
function populatePalettes() {
    ...
    var html = "";
    for (let pa of lJson) {
        html += generateListItemHtml('palette', pa[0], pa[1], 'setPalette', `<div class="lstIprev" style="${genPalPrevCss(pa[0])}"></div>`);
    }
    ...
    if (!isEmpty(lastinfo) && lastinfo.cpalcount) {
        for (let j = 0; j < lastinfo.cpalcount; j++) {
            ...
            div.outerHTML = generateListItemHtml('palette', 255 - j, '~ Custom ' + j + ' ~', 'setPalette', `<div class="lstIprev" style="${genPalPrevCss(255 - j)}"></div>`);
        }
    }
}
...
function generateListItemHtml(listName, id, name, clickAction, extraHtml = '', effectPar = '') {
    return `<div class="lstI${id == 0 ? ' sticky' : ''}" data-id="${id}" ${effectPar === '' ? '' : 'data-opt="' + effectPar + '" '}onClick="${clickAction}(${id})">` + `<label title="(${id})" class="radio schkl" onclick="event.preventDefault()">` + // (#1984)
        `<input type="radio" value="${id}" name="${listName}">` + `<span class="radiomark"></span>` + `<div class="lstIcontent">` + `<span class="lstIname">${name}</span>` + `</div>` + `</label>` + extraHtml + `</div>`;
}
...
function updatePA() {
    let ps;
    ps = gEBCN("pres");
    for (let p of ps) p.classList.remove('selected');
    ps = gEBCN("psts");
    for (let p of ps) p.classList.remove('selected');
    if (currentPreset > 0) {
        ...
        for (const fx of fxs) {
            ...
            if (fx.dataset.id > 0) {
                if (segLmax == 0) fx.classList.add('hide'); // none of the segments selected (hide all effects)
                    ...
                    if ((segLmax == 1 && (!opts[3] || opts[3].indexOf("0") < 0)) || (!isM && opts[3] && ((opts[3].indexOf("2") >= 0 && opts[3].indexOf("1") < 0)))) fx.classList.add('hide'); else fx.classList.remove('hide');
                }
            }
        }
...
function readState(s, command = false) {
    ...
    if (s.pl < 0) currentPreset = s.ps; else currentPreset = s.pl;
    ...
    if (s.seg.length > 2) d.querySelectorAll(".pop").forEach((e) => {
    ...
    for (let e = cd.length - 1; e >= 0; e--) {
        ...
        if (hasWhite || (!hasRGB && !hasWhite)) {
            cd[e].dataset.w = i.col[e][3];
        }
        setCSL(cd[e]);
    }
    selectSlot(csel);
...
function setEffectParameters(idx) {
    ...
    if (slOnOff.length > 5) { // up to 3 checkboxes
        ...
        for (let i = 0; i < 3; i++) {
            ...
            } else gId('opt' + i).classList.add('hide');
        }
    } else {
    ...
    for (let i = 0; i < gId("csl").children.length; i++) {
        ...
        if (coOnOff.length > i && coOnOff[i] != "") {
            ...
            } else if (i == 0) btn.innerHTML = "Fx"; else if (i == 1) btn.innerHTML = "Bg"; else btn.innerHTML = "Cs";
...
function makeSeg() {
    ...
    var cn = `<div class="seg lstI expanded">` + `<div class="segin">` + `<input type="text" id="seg${lu}t" autocomplete="off" maxlength=32 value="" placeholder="New segment ${lu}"/>` + `<table class="segt">` + `<tr>` + `<td width="38%">${isM ? 'Start X' : 'Start LED'}</td>` + `<td width="38%">${isM ? (cfg.comp.seglen ? "Width" : "Stop X") : (cfg.comp.seglen ? "LED count" : "Stop LED")}</td>` + `</tr>` + `<tr>` + `<td><input class="segn" id="seg${lu}s" type="number" min="0" max="${isM ? mw - 1 : ledCount - 1}" value="${ns}" oninput="updateLen(${lu})" onkeydown="segEnter(${lu})"></td>` + `<td><input class="segn" id="seg${lu}e" type="number" min="0" max="${ct}" value="${ct}" oninput="updateLen(${lu})" onkeydown="segEnter(${lu})"></td>` + `<td><button class="btn btn-xs" onclick="setSeg(${lu});"><i class="icons bth-icon" id="segc${lu}">&#xe390;</i></button></td>` + `</tr>` + `<tr id="mkSYH" class="${isM ? "" : "hide"}"><td>Start Y</td><td>${cfg.comp.seglen ? 'Height' : 'Stop Y'}</td></tr>` + `<tr id="mkSYD" class="${isM ? "" : "hide"}">` + `<td><input class="segn" id="seg${lu}sY" type="number" min="0" max="${mh - 1}" value="0" oninput="updateLen(${lu})" onkeydown="segEnter(${lu})"></td>` + `<td><input class="segn" id="seg${lu}eY" type="number" min="0" max="${mh}" value="${isM ? mh : 1}" oninput="updateLen(${lu})" onkeydown="segEnter(${lu})"></td>` + `</tr>` + `</table>` + `<div class="h" id="seg${lu}len">${ledCount - ns} LEDs</div>` + `<div class="c"><button class="btn btn-p" onclick="resetUtil()">Cancel</button></div>` + `</div>` + `</div>`;
    gId('segutil').innerHTML = cn;
}

function resetUtil(off = false) {
    gId('segutil').innerHTML = `<div class="seg btn btn-s${off ? ' off' : ''}" style="padding:0;">` + '<label class="check schkl"><input type="checkbox" id="selall" onchange="selSegAll(this)"><span class="checkmark"></span></label>' + `<div class="segname" ${off ? '' : 'onclick="makeSeg()"'}><i class="icons btn-icon">&#xe18a;</i>Add segment</div>` + '<div class="pop hide" onclick="event.stopPropagation();">' + `<i class="icons g-icon" onclick="this.nextElementSibling.classList.toggle('hide');">&#xE34B;</i>` + '<div class="pop-c hide"><span style="color:var(--c-f);" onclick="selGrp(0);">&#x278A;</span><span style="color:var(--c-r);" onclick="selGrp(1);">&#x278B;</span><span style="color:var(--c-g);" onclick="selGrp(2);">&#x278C;</span><span style="color:var(--c-l);" onclick="selGrp(3);">&#x278D;</span></div>' + '</div></div>';
}
...
function refreshPlE(p) {
    ...
    for (var i of sels) {
        if (i.dataset.val) {
            if (parseInt(i.dataset.val) > 0) i.value = i.dataset.val; else plJson[p].ps[i.dataset.index] = parseInt(i.value);
        }
    }
}
...
function pleDur(p, i, field) {
    if (field.validity.valid) plJson[p].dur[i] = Math.floor(field.value * 10);
}

function pleTr(p, i, field) {
    if (field.validity.valid) plJson[p].transition[i] = Math.floor(field.value * 10);
}
...
function rptSeg(s) {
    ...
    var obj = {
        "seg": {
            ...
            "n": name,
            "start": start,
            "stop": (cfg.comp.seglen ? start : 0) + stop,
            "rev": rev,
            "mi": mi,
            "on": pwr,
            "bri": parseInt(gId(`seg${s}bri`).value),
            "sel": sel
        }
    };
    if (gId(`seg${s}grp`)) {
...
function setSeg(s) {
    ...
    if (start < sX.min || start > sX.max) {
        sX.value = sX.min;
        return;
    } // prevent out of bounds
    if (stop < eX.min || stop - (cfg.comp.seglen ? start : 0) > eX.max) {
        eX.value = eX.max;
        return;
    } // prevent out of bounds
    if ((cfg.comp.seglen && stop == 0) || (!cfg.comp.seglen && stop <= start)) {
        delSeg(s);
        return;
    }
    ...
    if (isM && start < mw * mh) {
        ...
        if (startY < sY.min || startY > sY.max) {
            sY.value = sY.min;
            return;
        } // prevent out of bounds
        if (stopY < eY.min || stopY > eY.max) {
            eY.value = eY.max;
            return;
        } // prevent out of bounds
...
function saveP(i, pl) {
    ...
    setTimeout(() => {
        pmtLast = 0;
        loadPresets();
    }, 750); // force reloading of presets
}
...
function setPicker(rgb) {
    ...
    if (c.value > 0) cpick.color.set(c); else cpick.color.setChannel('hsv', 'v', 0);
    updateTrail(gId('sliderR'));
    updateTrail(gId('sliderG'));
    updateTrail(gId('sliderB'));
}
...
function loadPalettesData(callback = null) {
    ...
    getPalettesData(0, () => {
        localStorage.setItem(lsKey, JSON.stringify({
            p: palettesData, vid: lastinfo.vid
        }));
        redrawPalPrev();
        if (callback) setTimeout(callback, 99);
    });
...
function formatArr(pl) {
    ...
    if (l2 < l) {
        for (var i = 0; i < l - l2; i++) pl.dur.push(pl.dur[l2 - 1]);
    }
    ...
    if (l2 < l) {
        for (var i = 0; i < l - l2; i++) pl.transition.push(pl.transition[l2 - 1]);
    }
}
...
function expand(i) {
    ...
    if (i > 100) for (let p of ps) {
        p.classList.remove('selected');
        if (p !== seg) p.classList.remove('expanded');
...
function move(e) {
    ...
    if ((clientX != 0) && (iSlide > 0 || s < 0) && (iSlide < N - 1 || s > 0) && f > 0.12 && gEBCN("tabcontent")[iSlide].scrollTop == scrollS) {
