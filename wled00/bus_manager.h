...
class BusDigital : public Bus {
  ...
  BusDigital(BusConfig &bc, uint8_t nr) : Bus(bc.type, bc.start) {
    ...
    if (!ALLOCATE_PIN(bc.pins[0], true, PinOwner::BusDigital)) return;
    _pins[0] = bc.pins[0];
    if (IS_2PIN(bc.type)) {
      if (!ALLOCATE_PIN(bc.pins[1], true, PinOwner::BusDigital)) {
        cleanup(); return;
      }
      _pins[1] = bc.pins[1];
    }
  ...
  void cleanup() {
    ...
    _valid = false;
    _busPtr = nullptr;
    DEALLOCATE_PIN(_pins[1], PinOwner::BusDigital);
    DEALLOCATE_PIN(_pins[0], PinOwner::BusDigital);
  }
...
class BusPwm : public Bus {
  ...
  BusPwm(BusConfig &bc) : Bus(bc.type, bc.start) {
    ...
    for (uint8_t i = 0; i < numPins; i++) {
      ...
      if (!ALLOCATE_PIN(currentPin, true, PinOwner::BusPwm)) {
        deallocatePins(); return;
      }
  ...
  void deallocatePins() {
    ...
    for (uint8_t i = 0; i < numPins; i++) {
      ...
      #endif
      DEALLOCATE_PIN(_pins[i], PinOwner::BusPwm);
    }
    #ifdef ARDUINO_ARCH_ESP32
