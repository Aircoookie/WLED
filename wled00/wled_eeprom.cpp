...
void readStringFromEEPROM(uint16_t pos, char *str, uint16_t len) {
  ...
  str[len] = 0; // make sure every string is properly terminated. str must be at
                // least len +1 big.
}
...
void saveSettingsToEEPROM() {
  ...
  writeStringToEEPROM(2600, blynkServerAdr, 36);
  EEPROM.write(2637, blynkServerPort & 0xFF);
  EEPROM.write(2638, (blynkServerPort >> 8) & 0xFF);
...
// DMX (2530 - 2549)
  ...
  } // last used: 2549. maybe leave a few bytes for future expansion and go on
    // with 2600 kthxbye.
#endif
...
void loadSettingsFromEEPROM(bool first) {
  ...
  transitionDelayDefault =
      EEPROM.read(253) + ((EEPROM.read(254) << 8) & 0xFF00);
  ...
  if (lastEEPROMversion > 9) {
    ...
    strip.ablMilliampsMax =
        EEPROM.read(387) + ((EEPROM.read(388) << 8) & 0xFF00);
  } else if (lastEEPROMversion > 1) // ABL is off by default when updating from
                                    // version older than 0.8.2
  {
  ...
  } else if (strip.ablMilliampsMax ==
             65000) // 65000 indicates disabled ABL in <0.8.7
  {
    strip.ablMilliampsMax = ABL_MILLIAMPS_DEFAULT;
  ...
  readStringFromEEPROM(2600, blynkServerAdr, 36);
  blynkServerPort = EEPROM.read(2637) + ((EEPROM.read(2638) << 8) & 0xFF00);

  if (strlen(blynkApiKey) < 25) {
    blynkApiKey[0] = 0;
  }
  if (strlen(blynkServerAdr) < 8) {
    blynkServerAdr[0] = ' ';
  }
  ...
  // 2560 - 2943 usable, NOT reserved (need to increase EEPSIZE accordingly, new
  // WLED core features may override this section) 2944 - 3071 reserved for
  // Usermods (need to increase EEPSIZE to 3072 in const.h)
...
// 1:a 2:r 3:g 4:b 5:w 6:er 7:eg 8:eb 9:ew 10:fx 11:sx | custom chase 12:numP
// 13:numS 14:(0:fs 1:both 2:fe) 15:step 16:ix 17: fp 18-19:Zeros determines
// which presets already contain save data
void savedToPresets() {
...
bool applyPreset(byte index, bool loadBri) {
  ...
  if (index < 16) {
    ...
    strip.setColor(2, EEPROM.read(i + 12), EEPROM.read(i + 13),
                   EEPROM.read(i + 14), EEPROM.read(i + 15)); // tertiary color
...
void applyMacro(byte index) {
  ...
  if (!notifyMacro)
    mc += "&NN";
  String forbidden =
      "&M="; // dont apply if called by the macro itself to prevent loop
  /*
   * NOTE: loop is still possible if you call a different macro from a macro,
   * which then calls the first macro again. To prevent that, but also disable
   * calling macros within macros, comment the next line out.
   */
...
void saveMacro(byte index, const String &mc,
               bool persist) // only commit on single save, not in settings
{
  index -= 1;
  if (index > 15)
    return;
